...
export function LearningSuggestion() {
    ...
    const [wordList, setWordList] = useReducer('');
    const [topWord, setTopWord] = useReducer('');
    ...
    useEffect(() => {
        ...
        /*fetch('http://20.25.180.39/getwordlist', {
            ...
            .then((data) => {
                console.log("Success: ", data);
                data = data[0];
                councilSet.label = data.local;
                councilSet.data = [parseInt(data.total_landfill_per_household),
                                   parseInt(data.total_recycle_per_household),
                                   parseInt(data.total_organic_per_household)]
                setCouncilData(councilSet);
            })
            .catch((error) => {
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <h1></h1>
                <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                    <img src={imageUrl} alt="word image" style={{width: '50%', height: 'auto'}}/>
                </div>
                <div>
